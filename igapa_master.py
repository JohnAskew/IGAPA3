...
try:
    ...
    log_level, outlier_threshold, reports_hourly, reports_weekly = b.read_config_admin_reporting('.', 'config_admin.ini')
...
if len(HOURLY_TBLZ) > 0:
    ...
    if len(WEEKLY_TBLZ) > 0:

        log_and_print("Here is the WEEKLY Table: " +new_dir + '\\' + WEEKLY_TBLZ[0])
else:

    log_and_print("#####################################")

    log_and_print("# ERROR: Unable to access either: " + new_dir + "\\ WEEKLY_TABLE OR "  + new_dir + '\\HOURLY_TABLE')
...
if os.path.exists(test_dir): 
    ...
    msg_info = ("calling python subr_charts.py with host: " + str(new_dir)  + "  and config: " + str(pass_config))

    log_and_print(msg_info)

    subr_rc = subprocess.call(["python", dir_path + "/" + "subr_charts.py", str(new_dir), str(pass_config)])
